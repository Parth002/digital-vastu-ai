...
interface VastuFormProps {
  onFormSubmit: (file: File, entranceDirection: string, propertyType: 'residential' | 'commercial') => void;
  onReset?: () => void;
...
const VastuForm: React.FC<VastuFormProps> = ({ onFormSubmit, onReset, initialState, errorMessage }) => {
  ...
  const handleRemoveFile = useCallback((e: React.MouseEvent) => {
    ...
    if (fileInputRef.current) {
      fileInputRef.current.value = ''; // Reset the file input so user can re-select the same file
    }
  }, []);

  const handleSubmit = useCallback((e: React.FormEvent) => {
    e.preventDefault();
    if (!file || !entranceDirection || !propertyType) {
      setLocalError(t('form_error_missing_fields'));
      return;
    }
    setLocalError('');
    onFormSubmit(file, entranceDirection, propertyType);
  }, [file, entranceDirection, propertyType, onFormSubmit, t]);
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <div 
                ...
                className="relative mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-300 border-dashed rounded-md cursor-pointer hover:border-teal-500 transition-colors">
                 ...
                 {file ? (
                     <>
                        <button
                          ...
                          className="absolute top-2 right-2 z-10 bg-red-600 text-white rounded-full p-1.5 shadow-md hover:bg-red-700 transition-colors focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500"
                          aria-label={t('form_remove_file_label')}
                        >
                          <svg className="w-4 h-4" stroke="currentColor" fill="none" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M6 18L18 6M6 6l12 12" /></svg>
                        </button>
                        <div className="inline-block">
                          {filePreview ? (
                              <img src={filePreview} alt={t('form_alt_preview')} className="mx-auto h-40 object-contain rounded-md" />
                          ) : (
          ...
          <div>
            <label className="block text-lg font-semibold text-gray-700 mb-2">{t('form_property_type_label')}</label>
            <div className="mt-2 grid grid-cols-1 sm:grid-cols-2 gap-4">
                <div>
                    <input type="radio" id="residential" name="propertyType" value="residential" className="sr-only peer" checked={propertyType === 'residential'} onChange={(e) => setPropertyType(e.target.value as 'residential' | 'commercial')} required />
                    <label htmlFor="residential" className="flex items-center justify-center p-4 text-gray-600 bg-white border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50 peer-checked:border-teal-600 peer-checked:ring-2 peer-checked:ring-teal-500 peer-checked:text-teal-600 transition-all">
                        <span className="font-semibold">{t('property_type_residential')}</span>
                    </label>
                </div>
                <div>
                    <input type="radio" id="commercial" name="propertyType" value="commercial" className="sr-only peer" checked={propertyType === 'commercial'} onChange={(e) => setPropertyType(e.target.value as 'residential' | 'commercial')} />
                    <label htmlFor="commercial" className="flex items-center justify-center p-4 text-gray-600 bg-white border border-gray-300 rounded-lg cursor-pointer hover:bg-gray-50 peer-checked:border-teal-600 peer-checked:ring-2 peer-checked:ring-teal-500 peer-checked:text-teal-600 transition-all">
                        <span className="font-semibold">{t('property_type_commercial')}</span>
                    </label>
                </div>
            </div>
          </div>
          ...
          <div className="pt-2">
            <button
              ...
              disabled={!file || !entranceDirection || !propertyType}
            >
